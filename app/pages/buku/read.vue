...
const bookTitle = (route.query.title as string) || 'Membaca Buku Literatur'
const bookId = (route.query.id as string) || ''
const fileName = (route.query.fileName as string) || 'dokumen.pdf'
const fileUrl = (route.query.file as string) || ''

useHead({
  title: `${bookTitle} - Pembaca PDF Arsip Cendekia`
})
</script>
  ...
  <div class="space-y-4">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">
            ID: <span class="font-mono text-slate-300">{{ bookId }}</span> • Berkas: <span class="text-slate-300 font-mono">{{ fileName }}</span>
          </p>
        </div>
      </div>
    ...
    <div class="bg-[#0e1117] border border-slate-800/90 rounded-2xl p-6 min-h-[70vh] flex flex-col items-center justify-center text-center space-y-4 shadow-inner">
      ...
      <div class="p-3 bg-[#090b0e] border border-slate-800/80 rounded-xl text-xs text-slate-400 font-mono text-left max-w-sm w-full space-y-1">
        ...
        <div class="truncate text-slate-300">ID: {{ bookId || '-' }}</div>
        <div class="truncate text-slate-300">File: {{ fileName }}</div>
        <div class="truncate text-slate-400 text-[10px]">URL: {{ fileUrl || '(Supabase Storage Public URL)' }}</div>
      </div>
    </div>
  </div>
</template>
